...
def test_background_correction(
    ...
    if missing_binarization:
        ...
        logger.error("Please run binarization first for these FOVs")
        return False
...
def test_complete_workflow(
    ...
    """
    Test complete workflow on specified FOVs.
    ...
    logger.info(f"Testing complete workflow on FOVs {fov_start}-{fov_end}")
    ...
    # Connect logging signals for all services
    for service in coordinator.get_all_services():
        service.status_updated.connect(lambda msg, s=service: logger.info(f"[{s.get_step_name()}] {msg}"))
        service.error_occurred.connect(lambda msg, s=service: logger.error(f"[{s.get_step_name()}] {msg}"))
        service.progress_updated.connect(lambda p, s=service: logger.debug(f"[{s.get_step_name()}] Progress: {p}%"))
    ...
    # Run complete workflow with range support
    ...
    if success:
        ...
        logger.error(f"Complete workflow failed")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python test_modules.py binarization
  python test_modules.py background
  python test_modules.py traces
  python test_modules.py workflow -v    # Verbose output
...
Note: Edit the constants at the top of this script to configure test parameters."""
    ...
    parser.add_argument(
        "module",
        choices=["binarization", "background", "traces", "workflow"],
        help="Module to test"
    )
