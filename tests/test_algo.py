...
def demonstrate_feature_extraction(corrected_data, tracked_data, output_dir):
    ...
    for cell in sample_cells[:3]:
        ...
        print(f"  Cell {cell}: {len(cell_data)} timepoints, "
...
def demonstrate_model_fitting(df, output_dir):
    ...
    param_text = (f"$R^2$ = {result.r_squared:.3f}\n" +
                  "\n".join([f"{k} = {v:.3g}" for k, v in result.fitted_params.items()]))
    
    axs.text(0.05, 0.95, param_text,
...
def main():
    ...
    print(f"Processed {len(df.index.get_level_values('cell').unique())} cells "
