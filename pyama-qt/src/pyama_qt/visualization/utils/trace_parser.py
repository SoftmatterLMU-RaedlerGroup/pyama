...
class FeatureData:
    ...
    name: str
    cell_series: dict[str, np.ndarray] = field(
...
class PositionData:
    ...
    cell_positions: dict[str, dict[int, tuple[float, float]]] = field(
        default_factory=dict
    )
...
class TraceData:
    ...
    def __init__(self):
        ...
        self.features: dict[
            str, FeatureData
        ] = {}  # {feature_name: FeatureData instance}
    ...
    def get_trace_by_id(
        self, trace_id: str, trace_type: str = "intensity_total"
    ) -> np.ndarray | None:
        """
