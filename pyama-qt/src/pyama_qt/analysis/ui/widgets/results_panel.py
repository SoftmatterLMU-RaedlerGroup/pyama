...
class ResultsPanel(QWidget):
    ...
    def update_fitting_results(self, results_df: pd.DataFrame):
        ...
        if results_df is not None:
            r_squared_values = results_df.get("r_squared", pd.Series(dtype=float)).values
            if len(r_squared_values) > 0:
                colors = ["green" if r2 > 0.9 else "orange" if r2 > 0.7 else "red" for r2 in r_squared_values]
            ...
            for col in results_df.columns:
                if col not in metadata_cols:
                    ...
                    try:
                        ...
                        if pd.to_numeric(results_df[col], errors='coerce').notna().any():
    ...
    def update_param_histogram(self, param_name: str):
        ...
        values = pd.to_numeric(fitted_results[param_name], errors="coerce").dropna().values
        if len(values) > 0:
