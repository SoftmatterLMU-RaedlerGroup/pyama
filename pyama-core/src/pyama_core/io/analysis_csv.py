...
class AnalysisCSVWriter:
    ...
    def write_sample_data(self, df: pd.DataFrame, output_path: Path) -> None:
        ...
        if not self.validate_format(df):
            ...
            logger.error(
                f"{error_msg}. DataFrame shape: {df.shape}, index type: {type(df.index)}, columns: {list(df.columns)}"
            )
        ...
        try:
            ...
            try:
                ...
                error_msg = (
                    f"Permission denied creating directory {output_path.parent}: {e}"
                )
                logger.error(error_msg)
            ...
            try:
                df_to_write.to_csv(
                    output_path, index=True, header=True, float_format="%.6f"
                )
                ...
                error_msg = (
                    f"Unexpected error writing {output_path}: {type(e).__name__}: {e}"
                )
                logger.error(error_msg)
                raise IOError(error_msg) from e
            ...
            try:
                ...
                if file_size > 100 * 1024 * 1024:  # 100MB
                    logger.warning(
                        f"Large analysis CSV file created ({file_size / 1024 / 1024:.1f}MB): {output_path}"
                    )
            ...
            try:
                with open(output_path, "r") as f:
                    ...
                    if not first_line.startswith("time,"):
                        logger.warning(
                            f"Unexpected header format in {output_path}: {first_line[:50]}..."
                        )
            ...
            logger.info(
                f"Successfully wrote analysis CSV with {len(df)} time points and {len(df.columns)} cells to {output_path}"
            )
    ...
    def validate_format(self, df: pd.DataFrame) -> bool:
        ...
        if not all(isinstance(col, (int, str)) for col in df.columns):
            logger.error(
                "Column names must be integers or string representations of integers"
            )
            return False
        ...
        try:
            ...
            if actual_columns != expected_columns:
                logger.error(
                    f"Column names must be sequential integers starting from 0. Expected: {expected_columns}, Got: {actual_columns}"
                )
                return False
        except ValueError:
    ...
    def create_analysis_dataframe(
        self, time_values: list[float], cell_data: dict[int, list[float]]
    ) -> pd.DataFrame:
        """
    ...
    def load_analysis_csv(self, csv_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loaded analysis CSV with {len(df)} time points and {len(df.columns)} cells from {csv_path}"
            )
            return df
